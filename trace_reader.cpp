...
// This include provides the proper orcs_engine_t implementation needed by the
// macro ERROR_ASSERT_PRINTF. If you use clangd, it'll warn you about it. Try
// removing to see what happens.
// ~ Gabriel
...
void trace_reader_t::allocate(char *trace_file) {
    ...
    // =========================================================================
    file_name[0] = '\0';
    snprintf(file_name, sizeof(file_name), "%s.tid%d.stat.out.gz", trace_file,
             0);
    DEBUG_PRINTF("Static File = %s => READY !\n", file_name);
    // Open the .gz file.
    this->gzStaticTraceFile = gzopen(file_name, "ro");
    ERROR_ASSERT_PRINTF(gzStaticTraceFile != NULL,
                        "Could not open the static file.\n%s\n", file_name);
    DEBUG_PRINTF("Static File = %s => READY !\n", file_name);
    ...
    // =========================================================================
    file_name[0] = '\0';
    snprintf(file_name, sizeof(file_name), "%s.tid%d.dyn.out.gz", trace_file,
             0);
    // Open the .gz group.
    this->gzDynamicTraceFile = gzopen(file_name, "ro");
    ERROR_ASSERT_PRINTF(this->gzDynamicTraceFile != NULL,
                        "Could not open the dynamic file.\n%s\n", file_name);
    ...
    // =========================================================================
    file_name[0] = '\0';
    snprintf(file_name, sizeof(file_name), "%s.tid%d.mem.out.gz", trace_file,
             0);
    // Open the .gz group.
    this->gzMemoryTraceFile = gzopen(file_name, "ro");
    ERROR_ASSERT_PRINTF(this->gzMemoryTraceFile != NULL,
                        "Could not open the memory file.\n%s\n", file_name);
    ...
    ERROR_ASSERT_PRINTF(this->binary_bbl_size != NULL,
    ...
    ERROR_ASSERT_PRINTF(this->binary_dict != NULL,
                        "Could not allocate memory\n");
    for (uint32_t bbl = 1; bbl < this->binary_total_bbls; bbl++) {
        this->binary_dict[bbl] =
            new opcode_package_t[this->binary_bbl_size[bbl]];
        ERROR_ASSERT_PRINTF(this->binary_dict[bbl] != NULL,
                            "Could not allocate memory\n");
    }
...
void trace_reader_t::get_total_bbls() {
    ...
    while (!file_eof) {
        ...
        if (file_line[0] == '@') {
            ...
            ERROR_ASSERT_PRINTF(bbl == this->binary_total_bbls,
                                "Expected sequenced bbls.\n")
        }
    }
...
void trace_reader_t::generate_binary_dict() {
    ...
    while (!file_eof) {
        ...
        if (file_line[0] == '\0' || file_line[0] == '#') {
            ...
            BBL = (uint32_t)strtoul(file_line + 1, NULL, 10);
            ERROR_ASSERT_PRINTF(
                BBL < this->binary_total_bbls,
                "Static has more BBLs than previous analyzed static file.\n");
        } else {
            // If Inside BBL.
            DEBUG_PRINTF("Opcode %u = %s", opcode, file_line);
            this->trace_string_to_opcode(file_line,
                                         &this->binary_dict[BBL][opcode]);
            ERROR_ASSERT_PRINTF(
                this->binary_dict[BBL][opcode].opcode_address != 0,
                "Static trace file generating opcode address equal to zero.\n")
            opcode++;
        }
    }
};
...
// clang-format on
// =============================================================================
bool trace_reader_t::trace_string_to_opcode(char *input_string,
    ...
    opcode->opcode_operation =
        instruction_operation_t(strtoul(sub_string, NULL, 10));
    ...
    while (!valid_dynamic) {
        ...
        if (file_line[0] == '\0' || file_line[0] == '#') {
            ...
            *next_bbl = strtoul(file_line, NULL, 10);
            ERROR_ASSERT_PRINTF(*next_bbl != 0,
...
// clang-format on
// =============================================================================
bool trace_reader_t::trace_next_memory(uint64_t *mem_address,
                                       uint32_t *mem_size, bool *mem_is_read) {
    static char file_line[TRACE_LINE_SIZE];
    file_line[0] = '\0';
...
bool trace_reader_t::trace_fetch(opcode_package_t *m) {
    ...
    DEBUG_PRINTF("BBL:%u  OPCODE:%u = %s\n", this->currect_bbl,
                 this->currect_opcode, NewOpcode.opcode_assembly);
    ...
    *m = NewOpcode;
    ...
    if (m->is_read) {
        ...
        ERROR_ASSERT_PRINTF(mem_is_read == true,
                            "Expecting a read from memory trace\n");
    }

    if (m->is_read2) {
        trace_next_memory(&m->read_address, &m->read_size, &mem_is_read);
        ERROR_ASSERT_PRINTF(mem_is_read == true,
                            "Expecting a read2 from memory trace\n");
    }

    if (m->is_write) {
        trace_next_memory(&m->read_address, &m->read_size, &mem_is_read);
        ERROR_ASSERT_PRINTF(mem_is_read == false,
                            "Expecting a write from memory trace\n");
    }
